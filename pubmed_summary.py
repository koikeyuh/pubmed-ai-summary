...
class AIReporter:
    ...
    def summarize_with_japanese_title(self, abstract: str, title: str) -> Dict[str, any]:
        ...
        [出力条件]
            	...
            	2.	title_ja：30〜45字の邦題、名詞止め、冗長な副題は圧縮。原文タイトルの意図を正確に反映。
            	3.	bullets：最重要ポイントを4項目の配列で出力。
                	•	「目的・方法・結果・結論」の枠にこだわらず、原文で最も重要な4つの事実を選択。
                	•	各項目は簡潔に、かつ原文の情報を忠実に反映。
                	•	各ポイントは50-100文字程度
            	4.	専門性維持：放射線治療医が理解できる専門用語は原則そのまま使用。特に、Grade、Stage、dose、fraction、PTV、CTVなど、放射線腫瘍学で日常的に英語で用いられる用語は翻訳せず原文のまま記載。
