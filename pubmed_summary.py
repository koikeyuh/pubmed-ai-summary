...
class AIReporter:
    ...
    def summarize_with_japanese_title(self, abstract: str, title: str) -> Dict[str, any]:
        ...
        prompt = f"""
        [役割設定]
            あなたは、放射線治療の専門医に向けて、PubMed英語論文の内容を正確に日本語で要約する医療翻訳・要約者です。
            与えられた論文テキスト（タイトルと抄録）だけを情報源とし、推測や外部知識は一切使用せず、事実のみを正確に伝えてください。

        [入力形式]
            以下に英語の論文タイトルと抄録（または本文の一部）を提供します。
            この内容をもとに、放射線治療医が迅速に要点を把握できる形で日本語に要約してください。

        [出力条件]
            	1.	厳格JSONのみを出力（前後に余計な文字・説明・コードブロックを付けない）。
            	2.	title_ja：30〜45字の邦題、名詞止め、冗長な副題や補足（例：日本語訳、文字数などのメタ情報）は記載しない。原文タイトルの意図を正確に反映。
            	3.	bullets：最重要ポイントを4項目の配列で出力。
                	•	「目的・方法・結果・結論」の枠にこだわらず、原文で最も重要な4つの事実を選択。
                	•	各項目は簡潔に、かつ原文の情報を忠実に反映。
                	•	各ポイントは50-100文字程度
            	4.	専門用語の扱い：放射線治療医が理解できる範囲で日本語に訳す。ただし日本語に適切な訳語が存在しない場合のみ原文のまま残す（例：IMRT、PTV、CTV）。
            	5.	数字・統計・単位・p値は原文通りに正確に記載。
            	6.	意訳は最小限：意味を変える言い換えや情報の省略・追加はしない。日本語として自然にするための語順調整や文法修正のみ許可。
        ...
        prompt = f"""あなたは放射線腫瘍学の事実抽出専用サマライザーです。時間のない放射線治療医向けに最新論文を日本語要約をします。以下を厳守して、PubMedで見つかった論文の英語タイトルとアブストラクトから自然な日本語になるように要約をしてください。
                ...
                【出力の作り方】
                - "bullets" は4もしくは5点。各60〜120字。事実のみ、解釈や助言は書かない
